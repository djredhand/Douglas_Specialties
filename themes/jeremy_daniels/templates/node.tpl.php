	 ...
	 <?php
			...
			print_r('<div class="contact-sheet-wrapper">');
				for($i=0; $i<$numImages; $i++){
					...
					if (isset ($content['field_image'][$i]) ) {
						...
						print_r('<img title="Click to Enlarge" class="clone-image hide" src=" '. file_create_url($uri) . ' "/> ');
						print_r('<img title="Click to Enlarge" class="contact-sheet-image" src=" '. file_create_url($uri) . ' "/> ');
						print_r('<br>');
		...
		if ($type == 'home_gallery'){
			...
			for ($i=0;$i<$imgLength; $i++){
				$img = $base.'sites/default/files/'.$field_image[$i]["filename"];
				$imgArr['image'] = $img;
				array_push($imgObj, $imgArr);
			}//end for
			$all_Images = json_encode($imgObj);
			print_r('<script>var slideObj = '. $all_Images .'</script>');
			print_r('<script src="' . base_path() . 'themes/jeremy_daniels/js/supersized.3.2.7.js"></script>');
